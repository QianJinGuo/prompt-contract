 ...
 *
 * Two integration modes, both zero-dependency:
 ...
 *    client's OWN model — works with zero keys (PRD §6 调用模式一/二).
 *
 * Hotkey-path discipline applies here too: no middleware, no queue, config resolved once at startup.
 */
import { enhance, assembleMessages, hardConstraints, STRENGTHS, PromptBoostError, normalizeError } from '../../core/src/index.js';
...
export function createServer({ profiles, config, provider }) {
  const state = { profiles, config, provider, started: false };
  ...
  async function handle(msg) {
    ...
    if (method === 'tools/list') {
      return {
        tools: [{
          name: 'enhance_prompt',
          description: 'Rewrite a vague user prompt into a clear, specific, executable prompt. Returns only the enhanced text.',
          inputSchema: {
            type: 'object',
            properties: {
              text: { type: 'string', description: 'The raw user prompt to enhance' },
              profile: { type: 'string', enum: state.profiles.map((p) => p.name), description: 'Scenario profile (default: coding-agent)' },
    ...
    if (method === 'tools/call') {
      ...
      try {
        const profile = args.profile ? loadProfile(args.profile) : state.profiles.find((p) => p.name === 'coding-agent') ?? state.profiles[0];
    ...
    if (method === 'prompts/get') {
      ...
      const profile = state.profiles.find((p) => `boost-${p.name}` === name);
...
export async function serve({ stdin = process.stdin, stdout = process.stdout, stderr = process.stderr, argv = [] } = {}) {
  ...
  const config = resolveConfig({ provider: flag('--provider'), baseUrl: flag('--base-url'), apiKey: flag('--api-key'), model: flag('--model') });
  ...
  const server = createServer({ profiles, config, provider });
  stderr.write(`[prompt-boost] mcp server ready (provider=${config.provider}, profiles=${profiles.length})\n`);
