...
对应 PRD：`../prompt-contract-opensource-plan.md`（v1 修订版）。本文件是 M0 最小集（PRD §10）的实现映射与验收清单。
...
| `contract eval` 确定性规则断言部分 | ✅ 提前实现为 `contract check` | PRD 附录「硬约束进 eval 断言」——规则与模板共用同一规格，是本版验收标准本身 |
| `contract spike-0` macOS 取词/剪贴板恢复/焦点 dry-run 诊断 | ✅ 实现 | `packages/cli/src/spike-0.js` + `packages/cli/test/spike-0.test.js`；阈值与权限见 [SPIKE-0.md](SPIKE-0.md) |
| `contract watch` 快捷键常驻、浏览器插件、SDK 独立包、LLM-as-judge leaderboard | ❌ 不实现 | 决策记录 D7：Spike-0 只提供证据，不实现或自动解锁 watch；M1/M2 分期 |
...
| R7 | 场景 profiles ×3 + PR 即贡献（§3.2 V1） | `profiles/*.md`（frontmatter + 正文模板），`core/profile.js` 零依赖解析器；新增场景 = 新增 md 文件 | `profile.test.js` · `contract profiles` |
...
| R13 | CLI 形态（§4 P0） | `contract`：一次性增强（参数/stdin）、`--json`、`profiles`、`check`、`doctor`；`contract spike-0` 输出兼容性 JSON；`contract watch` 明示被 D7 门控 | `cli.test.js` e2e · `spike-0.test.js` |
...
| R16 | 验收标准即规则断言（附录） | `rules.js` 6 条确定性断言 = `contract check` = Playground 徽章 = `eval/run.mjs` 用例跑分 | `eval/run.mjs` 全 PASS |
| R17 | 隐私（§5.4） | BYOK、key 仅本地（env/config/localStorage）、遥测为零、依赖为零（无供应链面） | `package.json` 无 dependencies；代码无遥测调用 |

## 独立软件事实声明

- 本版不含 `contract watch`（D7 门控）：`contract watch` 会打印指向 Spike-0 证据与 D7 决策的说明并退出。
- `contract spike-0` 是 macOS-only、text-only clipboard 的 dry-run 诊断；它永不发送 ⌘V，因此不能证明真实 paste landing 正确。
...
基线 prompt vs 增强后 prompt，在同一批真实任务集、多个下游模型上对比：任务完成率、重试/追问次数、范围漂移、幻觉率、token 成本与端到端时延。LLM-as-judge 仅作为其中一个评分器，不作为唯一证据。确定性规则断言继续作为格式下限门禁（本仓库现有 `contract check`）。
