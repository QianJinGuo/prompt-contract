...
| `prompt-contract watch` 快捷键常驻、浏览器插件、SDK 独立包、LLM-as-judge leaderboard | ❌ 不实现 | 决策记录 D7：Spike-0 只提供证据，不实现或自动解锁 watch；M1/M2 分期 |
...
| R13 | CLI 形态（§4 P0） | `prompt-contract`：一次性增强（参数/stdin）、`--json`、`profiles`、`check`、`doctor`；`prompt-contract spike-0` 输出兼容性 JSON；`prompt-contract watch` 明示被 D7 门控 | `cli.test.js` e2e · `spike-0.test.js` |
...
- 本版不含 `prompt-contract watch`（D7 门控）：`prompt-contract watch` 会打印指向 Spike-0 证据与 D7 决策的说明并退出。
